...
from .appearance import themed_style
...
class SettingsView:
    def build_settings(self):
        ...
        body=QWidget();col=QVBoxLayout(body);col.setContentsMargins(0,0,12,12);col.setSpacing(20)
        box=QGroupBox('Appearance & workspace');form=QVBoxLayout(box)
        row=QHBoxLayout();row.addWidget(label('Color theme'));self.theme_choice=QComboBox();self.theme_choice.addItems(['Light','Dark']);self.theme_choice.setCurrentIndex(int(self.preferences.get('dark',False)));row.addWidget(self.theme_choice);row.addStretch();form.addLayout(row)
        ...
        form.addWidget(label('Turning telemetry off also stops background sensor polling. Reduced motion keeps the gradients still.','muted',True));col.addWidget(box)
        box=QGroupBox('Hugging Face account');form=QVBoxLayout(box);form.addWidget(label('Public model discovery works without an account. Sign in with a read token for your private or gated models.','muted',True));self.settings_account_label=label(self.account_label.text(),'muted',True);form.addWidget(self.settings_account_label);form.addWidget(self.hub_account_panel);self.hub_account_panel.show();col.addWidget(box)
        ...
        row=QHBoxLayout();row.addWidget(label('Default model context'));self.default_context=QSpinBox();self.default_context.setRange(512,8192);self.default_context.setSingleStep(512);self.default_context.setValue(self.preferences.get('context',2048));row.addWidget(self.default_context);row.addStretch();form.addLayout(row)
        form.addWidget(label('Defaults apply to the next model load. Active inference is unchanged. PC Assistant access controls remain beside its Start button so permissions are always visible.','muted',True))
        row=QHBoxLayout();row.addWidget(button('Device setup',lambda:self.nav.setCurrentRow(2)));row.addWidget(button('PC Assistant permissions',lambda:self.nav.setCurrentRow(6)));row.addWidget(button('Runtime diagnostics',lambda:self.nav.setCurrentRow(3)));form.addLayout(row);col.addWidget(box)
    ...
    def apply_appearance(self):
        ...
        self.dark=self.preferences.get('dark',False);self.setStyleSheet(themed_style(STYLE,self.dark))
        ...
        self.welcome_art.configure(self.dark,self.preferences.get('motion',True))
