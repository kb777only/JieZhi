...
class HubView:
    def build_hub(self):
        ...
        row.addWidget(button("Connect account", self.hub_connect, True)); row.addWidget(button("Disconnect", self.hub_disconnect)); account_layout.addLayout(row)
        row = QHBoxLayout()
        ...
        row.addWidget(self.hub_query, 1); row.addWidget(button("Search", self.hub_search, True)); row.addWidget(button("Open repository", self.hub_open_repo)); layout.addLayout(row)
        row=QHBoxLayout()
        self.hub_category=QComboBox()
        for key,title in CATEGORIES.items():self.hub_category.addItem(title,key)
        row.addWidget(label('Recommend for'));row.addWidget(self.hub_category,1)
        row.addWidget(label('Context'))
        self.hub_context=QComboBox()
        for size in [2048,4096,8192]:self.hub_context.addItem(f'{size//1024}K',size)
        self.hub_context.setCurrentIndex(1);row.addWidget(self.hub_context)
        row.addWidget(button('Refresh phone',self.hub_refresh_phone));layout.addLayout(row)
        self.hub_phone_label=label('', 'muted',True);layout.addWidget(self.hub_phone_label)
        ...
        self.nav.currentRowChanged.connect(lambda index:self.hub_search_timer.start() if index==4 and (not self.hub_phone.get('ram_gib') or not self.hub_repo_data and not self.hub_file_data) else None)
        ...
        for title, widget in [("MODELS · RANKED FOR YOUR PHONE", self.hub_repos), ("FILES · BEST FIT FIRST", self.hub_files)]:
            ...
            col.addWidget(label(title, "muted")); col.addWidget(widget); splitter.addWidget(pane)
        layout.addWidget(splitter, 1)
        layout.addWidget(label("Suitability ranks these results, not model quality. Speeds estimate decoding only; hover for assumptions. Compatibility still needs a load test.", "muted", True))
        row = QHBoxLayout()
        row.addWidget(button("Download to PC", lambda: self.hub_download(False), True))
        row.addWidget(button("Download && send to phone", lambda: self.hub_download(True)))
        row.addWidget(button("Model page ↗", self.hub_model_page)); row.addStretch()
        self.hub_pause = button("Pause", self.hub_cancel.set); self.hub_pause.setEnabled(False); row.addWidget(self.hub_pause); layout.addLayout(row)
        self.hub_progress = QProgressBar(); self.hub_progress.setValue(0); layout.addWidget(self.hub_progress)
        self.hub_note = label("Choose a category for suggestions, or type to search. Public models need no account.", "muted", True); layout.addWidget(self.hub_note)
        self.hub_render()
